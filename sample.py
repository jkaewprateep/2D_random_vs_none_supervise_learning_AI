...
################ Mixed of data input  ###############
global DATA
DATA = tf.zeros([1, 1, 1, n_blocks * 2 + n_blocks * 3 + 13 ], dtype=tf.float32)
global LABEL
LABEL = tf.zeros([1, 1, 1, 1], dtype=tf.float32)

for i in range(15):
	DATA_row = -9999 * tf.ones([1, 1, 1, n_blocks * 2 + n_blocks * 3 + 13], dtype=tf.float32)		
...
def	read_current_state( string_gamestate ):
	...
	if string_gamestate in ['snake_head_x']:
		...
		for i in range( n_blocks - int( len(values) ) ):
			temp = tf.concat([ temp, tf.zeros([1, ], dtype=tf.int32) ], axis=0 )

		temp = tf.constant( temp, shape=( n_blocks ), dtype=tf.int32 )
		temp = tf.cast( temp, dtype=tf.float32 )
		...
		for i in range( n_blocks - int( len(temp) ) ):
			temp = tf.concat([ temp, tf.zeros([1, 2], dtype=tf.int32) ], axis=0 )
		
		temp = tf.constant( temp, shape=( int( len(temp) ) * 2, ), dtype=tf.int32 )
		temp = tf.cast( temp, dtype=tf.float32 )
...
def update_DATA( action ):
	...
	for i in range( ( n_blocks * 2 + n_blocks * 3 + 13 ) - len( list_input ) ):
		list_input.append( 0 )
	...
	print( "steps: " + str( steps ).zfill(6) + " gamescores: " + str(gamescores) + " reward: " + str(int(reward)).zfill(6)

	)
	
	print( list_input )
	
	DATA_row = tf.constant([ list_input ], shape=(1, 1, 1, n_blocks * 2 + n_blocks * 3 + 13), dtype=tf.float32)	
...
for i in range(nb_frames):
	...
	if p.game_over() and gamescores < -50 :
		gamescores = 0
	...
	gamescores = gamescores + 10 * reward
